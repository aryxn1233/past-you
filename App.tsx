...
// Helper to convert Data URL to File object for the Web Share API
...
function App() {
    ...
    const handleShareIndividualImage = async (decade: string) => {
        ...
        if (image?.status === 'done' && image.url) {
            try {
                const file = dataURLtoFile(image.url, `past-forward-${decade}.png`);
                if (file && navigator.canShare && navigator.canShare({ files: [file] })) {
                    await navigator.share({
                        files: [file],
                        title: `My ${decade} look!`,
                        text: `Check out my ${decade} look generated by Past Forward!`,
                    });
                } else {
    ...
    const handleShareAlbum = async () => {
        ...
        try {
            ...
            const albumDataUrl = await createAlbumPage(imageData);
            const file = dataURLtoFile(albumDataUrl, 'past-forward-album.jpg');

            if (file && navigator.canShare && navigator.canShare({ files: [file] })) {
                await navigator.share({
                    files: [file],
                    title: 'My Past Forward Album',
                    text: 'I traveled through the decades with Past Forward! Check out my album.',
                });
            } else {
                alert("Sharing this album is not supported on your browser.");
            }
